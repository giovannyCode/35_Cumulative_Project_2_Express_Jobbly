...
describe("GET /users", function () {
  ...
  test("Show unauthorized  when user is not admin", async function () {
    const resp = await request(app)
      .get("/users")
...
describe("PATCH /users/:username", () => {
  ...
  test("Can edit for self user", async function () {
    const resp = await request(app)
      .patch(`/users/u2`)
      .send({
        firstName: "TestU2",
...
describe("DELETE /users/:username", function () {
  ...
  test("Can't delete when user request in url is different  from user logged in", async function () {
    const resp = await request(app)
      .delete(`/users/u3`)
      .set("authorization", `Bearer ${u2Token}`);
    expect(resp.statusCode).toEqual(401);
  ...
  test("Can delete when user request in url is the same as user logged in", async function () {
    const resp = await request(app)
      .delete(`/users/u2`)
      .set("authorization", `Bearer ${u2Token}`);
...
describe("Post /:username/jobs/:id", function () {
  test("works for users who are admin", async function () {
    const resp = await request(app)
      ...
      .set("authorization", `Bearer ${u1Token}`);
    console.log("userid", JobIds[0]);
    console.log("respinse", resp.body);
